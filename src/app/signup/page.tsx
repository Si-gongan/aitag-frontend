...
export default function Signup() {
  ...
  const handleServiceTermsChange = (value: string) => {
    setServiceTermsAccepted(!serviceTermsAccepted);
  };
  const handlePersonalInfoChange = (value: string) => {
    setPersonalInfoAccepted(!personalInfoAccepted);
  };
    ...
    <div className="flex items-center justify-center min-h-screen bg-[#FAFBFC] px-6 ">
      <div className="w-full h-3/5 max-w-4xl">
        ...
        <div className="mb-50">
          ...
          {errorMessage !== '비밀번호가 일치하지 않습니다.' && <AlertDanger message={errorMessage} />}
        </div>
        <form className="px-8 pt-6 pb-8 mb-4" onSubmit={handleSubmit}>
          <div className="mb-20">
            <h1 className="text-14 font-bold text-left mb-3">이름 / 기업명 </h1>
            <input
              className="border h-53 rounded-lg w-full py-10 px-15 text-gray-700 text-16 focus:outline-none"
              id="name"
              type="text"
              placeholder="이름 혹은 기업명을 입력해주세요"
          ...
          <div className="flex items-center mb-16">
            <Checkbox value="allTerms" checked={termsAccepted} handleCheck={handleTermsChange} />
            <span className="ml-10 text-16 text-gray-700">전체 약관에 동의합니다</span>
          </div>
          <div className="flex items-center mb-16">
            <Checkbox value="ageTerm" checked={ageConfirmed} handleCheck={handleAgeChange} />
            <span className="ml-10 text-16 text-gray-700">만 14세 이상입니다</span>
          </div>
          <div className="flex items-center mb-16">
            <Checkbox value="service" checked={serviceTermsAccepted} handleCheck={handleServiceTermsChange} />
            <span className="ml-10 text-16 text-gray-700">
              (필수) 에이택 웹사이트 이용약관{' '}
              <button type="button" className="text-blue-500 hover:underline ml-10" onClick={handleModalOpen}>
                보기
              </button>
            </span>
          </div>
          <div className="flex items-center mb-16">
            <Checkbox value="personal" checked={personalInfoAccepted} handleCheck={handlePersonalInfoChange} />
            <span className="ml-10 text-16 text-gray-700">
              (필수) 에이택 웹사이트 개인정보 처리방침{' '}
              <button type="button" className="text-blue-500 hover:underline ml-10" onClick={handleModalOpen}>
                보기
              </button>
            </span>
          ...
          <button
            className={`w-full h-53 text-16 py-8 rounded-lg focus:outline-none mb-50 ${termsAccepted && ageConfirmed && serviceTermsAccepted && personalInfoAccepted ? 'bg-[#4C80F1] hover:bg-blue-700 text-white' : 'bg-gray-400 text-gray-700 cursor-not-allowed'}`}
            type="submit"
            disabled={!termsAccepted || !ageConfirmed || !serviceTermsAccepted || !personalInfoAccepted}>
            가입하기
          </button>
        </form>
